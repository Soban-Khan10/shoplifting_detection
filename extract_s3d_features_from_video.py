import argparse
import json
import sys
from pathlib import Path

import cv2
import numpy as np
import torch
import torch.nn.functional as F
from torchvision.models.video import S3D_Weights, s3d


FEATURE_DIM = 1024
KINETICS_MEAN = np.array([0.43216, 0.394666, 0.37645], dtype=np.float32)
KINETICS_STD = np.array([0.22803, 0.22145, 0.216989], dtype=np.float32)


def parse_args():
    parser = argparse.ArgumentParser(
        description="Extract real 1024-d S3D video features from an input video."
    )
    parser.add_argument("--video", required=True, help="Path to the input .mp4 video.")
    parser.add_argument("--output", required=True, help="Path to save output .npy features.")
    parser.add_argument("--clip-len", type=int, default=32, help="Number of frames per clip.")
    parser.add_argument("--stride", type=int, default=16, help="Frame stride between clip starts.")
    parser.add_argument("--resize", type=int, default=256, help="Square resize size before center crop.")
    parser.add_argument("--crop-size", type=int, default=224, help="Square center crop size.")
    parser.add_argument(
        "--device",
        default="auto",
        choices=["auto", "cpu", "mps"],
        help="Inference device. Default uses MPS when available, otherwise CPU.",
    )
    parser.add_argument(
        "--weights",
        default="KINETICS400_V1",
        choices=["KINETICS400_V1", "none"],
        help="S3D pretrained weights to use. Use 'none' only for debugging untrained features.",
    )
    return parser.parse_args()


def select_device(device_name):
    if device_name == "auto":
        return torch.device("mps" if torch.backends.mps.is_available() else "cpu")
    if device_name == "mps" and not torch.backends.mps.is_available():
        raise RuntimeError("Device 'mps' was requested, but MPS is not available.")
    return torch.device(device_name)


def validate_args(video_path, output_path, clip_len, stride, resize, crop_size):
    if not video_path.exists():
        raise FileNotFoundError(f"Video file not found: {video_path}")
    if output_path.suffix.lower() != ".npy":
        raise ValueError(f"--output must end in .npy, got: {output_path}")
    if clip_len <= 0:
        raise ValueError(f"--clip-len must be greater than zero, got {clip_len}")
    if stride <= 0:
        raise ValueError(f"--stride must be greater than zero, got {stride}")
    if resize <= 0:
        raise ValueError(f"--resize must be greater than zero, got {resize}")
    if crop_size <= 0:
        raise ValueError(f"--crop-size must be greater than zero, got {crop_size}")
    if resize < crop_size:
        raise ValueError(f"--resize must be >= --crop-size, got resize={resize}, crop_size={crop_size}")


def metadata_path_for(output_path):
    return output_path.with_suffix(".metadata.json")


def center_crop(frame, crop_size):
    height, width = frame.shape[:2]
    top = max(0, (height - crop_size) // 2)
    left = max(0, (width - crop_size) // 2)
    return frame[top:top + crop_size, left:left + crop_size]


def read_preprocessed_frames(video_path, resize, crop_size):
    cap = cv2.VideoCapture(str(video_path))
    if not cap.isOpened():
        raise RuntimeError(f"Cannot open video: {video_path}")

    frames = []
    try:
        original_fps = float(cap.get(cv2.CAP_PROP_FPS))
        total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))

        while True:
            ok, frame_bgr = cap.read()
            if not ok:
                break
            frame_rgb = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)
            frame_rgb = cv2.resize(frame_rgb, (resize, resize), interpolation=cv2.INTER_AREA)
            frame_rgb = center_crop(frame_rgb, crop_size)
            frames.append(frame_rgb)
    finally:
        cap.release()

    if not frames:
        raise RuntimeError(f"No frames could be read from video: {video_path}")

    return np.stack(frames, axis=0).astype(np.uint8), original_fps, total_frames


def build_clips(frames, clip_len, stride):
    clips = []
    for start in range(0, frames.shape[0] - clip_len + 1, stride):
        clips.append(frames[start:start + clip_len])
    if not clips:
        raise RuntimeError(
            f"No clips created: video has {frames.shape[0]} usable frames, "
            f"but clip_len={clip_len} and stride={stride}."
        )
    return clips


def preprocess_clip(clip):
    clip = clip.astype(np.float32) / 255.0
    clip = (clip - KINETICS_MEAN.reshape(1, 1, 1, 3)) / KINETICS_STD.reshape(1, 1, 1, 3)
    tensor = torch.from_numpy(clip).permute(3, 0, 1, 2).unsqueeze(0)
    return tensor.contiguous()


def load_s3d_model(weights_name, device):
    weights = None
    if weights_name == "KINETICS400_V1":
        weights = S3D_Weights.KINETICS400_V1
    try:
        model = s3d(weights=weights)
    except Exception as exc:
        raise RuntimeError(
            "Could not load torchvision S3D pretrained weights. "
            "If the weights are not cached locally, allow the download or place "
            "S3D_Weights.KINETICS400_V1 in the Torch cache. "
            f"Original error: {exc}"
        ) from exc

    model.eval()
    model.to(device)
    return model


def extract_features(model, clips, device):
    features = []
    with torch.no_grad():
        for clip in clips:
            tensor = preprocess_clip(clip).to(device)
            feature_map = model.features(tensor)
            pooled = F.adaptive_avg_pool3d(feature_map, output_size=(1, 1, 1))
            feature = pooled.flatten(1).squeeze(0).cpu().numpy()
            if feature.shape[0] != FEATURE_DIM:
                raise RuntimeError(
                    f"Expected S3D feature dimension {FEATURE_DIM}, got {feature.shape[0]}."
                )
            features.append(feature)

    output = np.stack(features, axis=0).astype(np.float32)
    if output.ndim != 2 or output.shape[1] != FEATURE_DIM:
        raise RuntimeError(f"Expected output feature shape (T, {FEATURE_DIM}), got {output.shape}.")
    return output


def save_metadata(
    metadata_path,
    video_path,
    original_fps,
    total_frames,
    used_frame_count,
    clip_len,
    stride,
    num_features,
    feature_dim,
    device,
    weights_name,
    output_path,
):
    metadata_path.parent.mkdir(parents=True, exist_ok=True)
    metadata = {
        "video_path": str(video_path),
        "original_fps": original_fps,
        "total_frames": total_frames,
        "sampled_or_used_frame_count": used_frame_count,
        "clip_len": clip_len,
        "stride": stride,
        "number_of_clips_features": num_features,
        "feature_dim": feature_dim,
        "device": str(device),
        "weights_used": weights_name,
        "output_feature_path": str(output_path),
        "preprocessing": {
            "color": "RGB",
            "resize": "square",
            "center_crop": True,
            "normalization_mean": KINETICS_MEAN.tolist(),
            "normalization_std": KINETICS_STD.tolist(),
        },
        "compatibility_warning": (
            "These are torchvision S3D Kinetics features. They may not exactly match "
            "the original UCF I3D-style features used to train the existing anomaly models."
        ),
    }
    with metadata_path.open("w") as f:
        json.dump(metadata, f, indent=2)


def main():
    args = parse_args()
    video_path = Path(args.video).expanduser()
    output_path = Path(args.output)
    metadata_path = metadata_path_for(output_path)

    validate_args(video_path, output_path, args.clip_len, args.stride, args.resize, args.crop_size)
    device = select_device(args.device)

    frames, original_fps, total_frames = read_preprocessed_frames(
        video_path=video_path,
        resize=args.resize,
        crop_size=args.crop_size,
    )
    clips = build_clips(frames, clip_len=args.clip_len, stride=args.stride)
    model = load_s3d_model(args.weights, device)
    features = extract_features(model, clips, device)

    output_path.parent.mkdir(parents=True, exist_ok=True)
    np.save(output_path, features)
    save_metadata(
        metadata_path=metadata_path,
        video_path=video_path,
        original_fps=original_fps,
        total_frames=total_frames,
        used_frame_count=int(frames.shape[0]),
        clip_len=args.clip_len,
        stride=args.stride,
        num_features=int(features.shape[0]),
        feature_dim=int(features.shape[1]),
        device=device,
        weights_name=args.weights,
        output_path=output_path,
    )

    print(f"Input video path: {video_path}")
    print(f"Device: {device}")
    print(f"Number of clips: {features.shape[0]}")
    print(f"Output feature shape: {features.shape}")
    print(f"Saved feature path: {output_path}")
    print(f"Saved metadata path: {metadata_path}")


if __name__ == "__main__":
    try:
        main()
    except (FileNotFoundError, ValueError, RuntimeError, OSError) as exc:
        print(f"Error: {exc}", file=sys.stderr)
        sys.exit(1)
