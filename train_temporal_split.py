...
OUTPUT_CHECKPOINT_PATH = Path("anomaly_net_temporal_split_weights.pth")
...
def split_bases(bases, train_ratio, seed):
    shuffled = sorted(bases)
    ...
    train_bases = sorted(shuffled[:split_idx])
    eval_bases = sorted(shuffled[split_idx:])
...
class TemporalSplitDataset(Dataset):
    def __init__(self, anomaly_grouped, normal_grouped, annotations, train_anomaly_bases, train_normal_bases):
        self.samples = []
        ...
        if not self.samples:
            raise ValueError("No temporal split fine-tuning samples found.")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train temporal supervised fine-tuning with a base-video split.")
...
def train():
    ...
    annotated_anomaly_bases = sorted(base for base in anomaly_grouped if base in annotations)
    missing_annotation_bases = sorted(base for base in anomaly_grouped if base not in annotations)
    if missing_annotation_bases:
        print(f"Skipping anomaly bases without Shoplifting annotations: {missing_annotation_bases}")

    train_anomaly_bases, eval_anomaly_bases = split_bases(
        annotated_anomaly_bases,
        args.train_ratio,
        args.seed,
    )
    train_normal_bases, eval_normal_bases = split_bases(
        normal_grouped.keys(),
        args.train_ratio,
        args.seed,
    )
    ...
    split = {
        ...
        "eval_anomaly_bases": eval_anomaly_bases,
        "train_normal_bases": train_normal_bases,
        "eval_normal_bases": eval_normal_bases,
    }
    ...
    torch.save(
        {
            ...
            "training_type": "temporal_supervised_split_finetune",
    ...
    print(f"Saved split path: {SPLIT_PATH}")
